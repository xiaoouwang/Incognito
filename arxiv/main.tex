...
\title{%
  \textbf{Text Data Anonymizer:}\\[0.3em]
  \large A Local-First, Cross-Platform Desktop Tool for\\
  Interactive Anonymization of French Qualitative Text
}
...
We present \textbf{Text Data Anonymizer}, a cross-platform desktop application
...
French text entirely on the user's machine.
The tool combines a React/Electron front end with a persistent local Python NER
...
named entity recognition, French NLP, local-first software, research ethics
...
\textbf{Text Data Anonymizer} addresses this gap as a research prototype and
distribution-ready desktop application maintained in the context of research
...
\begin{itemize}[noitemsep]
  ...
  \item \textbf{Cross-platform deployment:} the same workflow on macOS, Windows,
        and Linux through Electron and platform-specific installers.
...
Text Data Anonymizer follows a two-process architecture (Figure~\ref{fig:architecture}).
...
application is the primary interactive path developed for MSHS Sud-Est researchers.
...
\paragraph{Releases.}
Standalone binaries (macOS DMG/ZIP for Apple Silicon and Intel, Windows installer,
...
\small
Wang, X. (2026).
\textit{Text Data Anonymizer} (Version 0.1.0) [Computer software].
Maison des Sciences de l'Homme Sud-Est, Universit\'{e} C\^{o}te d'Azur.
\url{https://github.com/xiaoouwang/anonymizer}
\end{quote}
\noindent Bib\TeX{} key: \texttt{wang2026textdataanonymizer}.
